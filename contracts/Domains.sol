...
contract Domains is ERC721URIStorage, Donate, Links {
    ...
    event NewRegisteredDomain(address _from, string _domain, uint256 _amount);

    constructor(string memory _tld) payable ERC721("LinkMe", "LME") {
        tld = _tld;
        console.log("%s deployed", _tld);
    }
    ...
    function ValidHandle(string calldata name) public view returns (bool) {
        if (domains[name] != address(0)) return false;
        return true;
    }
